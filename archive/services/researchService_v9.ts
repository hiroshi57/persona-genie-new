...
export class ResearchService {
  ...
  private evaluateCondition(persona: PersonaDetails, cond: ScreeningCondition): boolean {
    ...
    switch (cond.field) {
      case 'age': {
        const age = parseInt(basicInfo['年齢']);
...
${Object.entries(insights.segmentedMarketSize)
   ...
   * 市場に応じたデフォルトアンケートを生成
   */
  generateDefaultSurvey(market: PredefinedMarket): Survey {
    const surveyMap: Record<string, Survey> = {
      'pet-supplies': {
        id: `survey-${market.id}-${Date.now()}`,
        name: `${market.name} アンケート`,
        description: `${market.name}に関する購買行動調査`,
        marketId: market.id,
        questions: [
          {
            id: 'q1',
            type: 'single',
            text: 'ペット用品をどこで購入することが多いですか？',
            options: ['オンラインストア', 'ペットショップ', 'ホームセンター', 'スーパー'],
          } as SingleAnswerQuestion,
          {
            id: 'q2',
            type: 'multi',
            text: '購入時に重視する点は？',
            options: ['価格', '品質', 'ブランド', '機能性', '配送速度'],
          } as MultiAnswerQuestion,
          {
            id: 'q3',
            type: 'matrix',
            text: '以下の商品タイプへの興味度を教えてください',
            rowLabels: ['フード', 'トイズ', 'ケア用品', 'アパレル'],
            colLabels: ['全く興味なし', 'あまり興味ない', '普通', 'やや興味ある', 'とても興味ある'],
          } as MatrixQuestion,
        ],
        createdAt: Date.now(),
      },
        ...
        questions: [
          {
            ...
            type: 'single',
            text: 'スキンケア製品の購入頻度は？',
            options: ['月1回以下', '月2-3回', '週1回', '週2回以上'],
          } as SingleAnswerQuestion,
          {
            id: 'q2',
            type: 'multi',
            text: '美容製品購入時の情報源は？',
            options: ['SNS', 'ブログ', '口コミサイト', 'テレビCM', 'インフルエンサー'],
          } as MultiAnswerQuestion,
          {
            id: 'q3',
            type: 'matrix',
            text: '美容成分への関心度',
            rowLabels: ['ヒアルロン酸', 'セラミド', 'コラーゲン', 'ビタミンC', 'プラセンタ'],
            colLabels: ['全く興味なし', 'あまり興味ない', '普通', 'やや興味ある', 'とても興味ある'],
          } as MatrixQuestion,
        ],
        createdAt: Date.now(),
      ...
      'finance': {
        id: `survey-${market.id}-${Date.now()}`,
        name: `${market.name} アンケート`,
        description: `${market.name}に関する利用行動調査`,
        marketId: market.id,
        questions: [
          {
            id: 'q1',
            type: 'single',
            text: 'よく利用する金融サービスの種類は？',
            options: ['銀行', '証券会社', 'FX', 'クレジットカード', 'デジタルウォレット'],
          } as SingleAnswerQuestion,
          {
            id: 'q2',
            type: 'multi',
            text: '金融サービス選択時の重視点は？',
            options: ['手数料', '利便性', 'セキュリティ', 'サポート', '利回り'],
          } as MultiAnswerQuestion,
          {
            id: 'q3',
            type: 'matrix',
            text: '各金融商品への関心度',
            rowLabels: ['定期預金', '投資信託', '株式', '仮想資産', 'ロボアド'],
            colLabels: ['全く興味なし', 'あまり興味ない', '普通', 'やや興味ある', 'とても興味ある'],
          } as MatrixQuestion,
        ],
        createdAt: Date.now(),
      ...
      'insurance': {
        id: `survey-${market.id}-${Date.now()}`,
        name: `${market.name} アンケート`,
        description: `${market.name}に関する加入行動調査`,
        marketId: market.id,
        questions: [
          {
            id: 'q1',
            type: 'single',
            text: '現在加入している保険の種類は？',
            options: ['生命保険', '医療保険', '損害保険', '介護保険', '複数加入'],
          } as SingleAnswerQuestion,
          {
            id: 'q2',
            type: 'multi',
            text: '保険選択時に重視する要因は？',
            options: ['保障内容', '掛金', '手続きの簡単さ', 'ブランド', 'カスタマーサポート'],
          } as MultiAnswerQuestion,
          {
            id: 'q3',
            type: 'matrix',
            text: '各保険商品への関心度',
            rowLabels: ['定期保険', '終身保険', 'がん保険', '介護保険', 'ペット保険'],
            colLabels: ['全く興味なし', 'あまり興味ない', '普通', 'やや興味ある', 'とても興味ある'],
          } as MatrixQuestion,
        ],
        createdAt: Date.now(),
    ...
    return surveyMap[market.id] || surveyMap['pet-supplies'];
  }
}
